...
class MovingAverageCrossStrategy(IStrategy):
    ...
    use_custom_stoploss = False  # 禁用 custom stoploss，避免 trailing stop 导致的大量亏损
    ...
    # 移动平均线类型
    ma_type_fast = IntParameter(0, 8, default=1, space="buy")  # 0=SMA, 1=EMA, 2=WMA等
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                dataframe['ma_cross_up'] &                              # 金叉信号
                (dataframe['close'] > dataframe['trend_ema']) &         # 价格在长期趋势之上
                (dataframe['di_plus'] > dataframe['di_minus']) &        # DI确认上升趋势
                (dataframe['rsi'] < 80)                                  # RSI不能过度超买
            ),
            'enter_long'
        ] = 1
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                # 主要退出：死叉 + ADX弱化（需要2个条件同时满足）
                (conditions[0] & conditions[1]) |

                # 或者：趋势明确转向（快线下降 + MACD转弱）
                (conditions[2] & conditions[4]) |

                # 或者：明确跌破趋势线 + DI转向
                (conditions[6] & conditions[3])
            ),
            'exit_long'
        ] = 1
